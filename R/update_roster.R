...
update_roster <- function(current_df, previous_df = NULL, verbose = TRUE) {
  ...
  if (length(missing_cols) > 0) {
    stop(
      "current_df is missing required columns: ",
  ...
  if (!is.null(previous_df)) {
    ...
    if (length(missing_cols_prev) > 0) {
      stop(
        "previous_df is missing required columns: ",
        paste(missing_cols_prev, collapse = ", ")
      )
    }
  }
  ...
  if (is.null(previous_df)) {
    ...
    if (is.null(unit)) {
      stop(
        "Unit attribute not found in current_df. Please provide previous_df or ensure parse_roster was called with unit."
      )
    }
    ...
    # Ensure lab is character
    previous_df$lab <- as.character(previous_df$lab)
    # Ensure subject_activitycode is character
  ...
  removals <- full_join_df |>
    ...
    select(
      all_of(key_cols),
      name = name_prev,
      rate_code = rate_code_prev,
      subject_activitycode = subject_activitycode_prev
    )
  ...
  if (total_changes > 0) {
    ...
    save_roster(
      ...
      logs_dir,
      glue::glue(
        "Roster changes detected ({total_changes} total) - saving updated roster"
      )
    )
  } else {
...
summary.roster_changes <- function(object, ..., html = FALSE) {
  ...
  get_details <- function(row, type) {
    ...
    switch(
      type,
      "Addition" = paste0(
        cli::style_bold(cli::col_blue(row$name)),
        " added (",
        role,
        ")"
      ),
      ...
      "Replacement" = paste0(
        cli::style_strikethrough(cli::style_bold(cli::col_red(
          row$previous_name
        ))),
        " → ",
        cli::style_bold(cli::col_blue(row$current_name)),
        " (",
        role,
        ")"
      ),
  ...
  get_details_html <- function(row, type) {
    ...
    switch(
      ...
      "Replacement" = paste0(
        "<del style='color: red; font-weight: bold;'>",
        row$previous_name,
        "</del> &rarr; <span style='color: blue; font-weight: bold;'>",
        row$current_name,
        "</span> (",
        role,
        ")"
  ...
  build_table_html <- function(changes_df, type_label) {
    ...
    for (wk in sort(weeks)) {
      ...
      table_html <- paste0(
        table_html,
        "<tr style='background-color: #f0f0f0;'><th style='padding: 8px;'>Date</th><th style='padding: 8px;'>Type</th><th style='padding: 8px;'>Details</th><th style='padding: 8px;'>Activity</th></tr>"
      )
      ...
      for (i in seq_len(nrow(wk_changes))) {
        ...
        activity <- paste0(
          subject,
          " <strong>",
          sprintf("%02d", as.integer(activity_code)),
          "</strong>: ",
          lab_num,
          " ",
          row$day_of_week,
  ...
  if (html) {
    ...
    html_content <- paste0(
      ...
      if (total_additions > 0) {
        paste0(
          "<h2>Additions</h2>",
          build_table_html(object$additions, "Addition")
        )
      } else {
        ""
      },
      if (total_removals > 0) {
        paste0(
          "<h2>Removals</h2>",
          build_table_html(object$removals, "Removal")
        )
      } else {
        ""
      },
      if (total_replacements > 0) {
        paste0(
          "<h2>Replacements</h2>",
          build_table_html(object$replacements, "Replacement")
        )
      } else {
        ""
      },
      if (total_rate_changes > 0) {
        paste0(
          "<h2>Rate changes</h2>",
          build_table_html(object$rate_changes, "Rate Change")
        )
      } else {
        ""
      },
    ...
    # Always save to the logs folder
    ...
    if (
      ...
      message("HTML summary saved to ", get_relative_path(html_path), " and displayed in RStudio viewer.")
    } else {
      relative_path <- get_relative_path(html_path)
      message(glue::glue(
        "HTML summary saved to '{relative_path}'. Open in your editor for preview."
      ))
    }
  }
    ...
    for (wk in sort(weeks)) {
      ...
      for (i in seq_len(nrow(wk_changes))) {
        ...
        activity <- paste0(
          subject,
          " ",
          cli::style_bold(sprintf("%02d", as.integer(activity_code))),
          ": ",
          lab_num,
          " ",
          row$day_of_week,
      ...
      col_widths <- pmax(
        col_widths,
        c(Date = 10, Type = 12, Details = 20, Activity = 15)
      )
      ...
      for (i in seq_len(nrow(table_df))) {
        cat(
          pad_string(table_df$Date[i], col_widths["Date"]),
          " | ",
